...
Big thanks to @tfoldi (F철ldi Tam찼s) and @legion1581 (The RoboVerse Discord Group)
"""
...
class Go2Connection:
    ...
    def on_data_channel_open(self) -> None:
        ...
        # Workaround: force readyState if needed
        try:
            if getattr(self.data_channel, "readyState", "") != "open":
                self.data_channel._setReadyState("open")  # type: ignore[attr-defined]
        except Exception:
            pass
    ...
    def publish(self, topic: str, data: Any, msg_type: str = "msg") -> None:
        ...
        try:
            if getattr(self.data_channel, "readyState", "") != "open":
                logger.warning(f"Data channel is not open. State is {getattr(self.data_channel, 'readyState', '')}")
                return
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # Step 2: Get robot's public key (con_notify)
            ...
            raise Go2ConnectionError(f"Failed during encrypted handshake: {e}") from e
        except Exception as e:
            raise Go2ConnectionError(f"Unexpected error during connection: {e}") from e

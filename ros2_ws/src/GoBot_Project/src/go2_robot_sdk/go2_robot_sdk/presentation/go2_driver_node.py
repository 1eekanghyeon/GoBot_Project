...
class Go2DriverNode(Node):
    ...
    def _setup_subscribers(self) -> None:
        ...
        if self.config.conn_mode == 'single':
            ...
            self.create_subscription(
                String,
                'tts_audio_stream',
                lambda msg: self._on_tts_audio(msg, "0"), qos_profile)
            ...
            for i in range(num_robots):
                ...
                self.create_subscription(
                    String,
                    f'robot{i}/tts_audio_stream',
                    lambda msg, robot_id=str(i): self._on_tts_audio(msg, robot_id), qos_profile)
    ...
    def _on_tts_audio(self, msg: String, robot_id: str) -> None:
        """
        Called when TTS node publishes synthesized speech audio
        that should play from the robot's speaker.

        msg.data (String) is expected to be a JSON string like:
        {
            "duration": 1.23,
            "mp3_b64": "<base64-encoded mp3 bytes>"
        }
        """
        try:
            payload = json.loads(msg.data)

            duration_sec = float(payload["duration"])
            mp3_b64 = payload["mp3_b64"]

            # base64 -> raw mp3 bytes
            mp3_bytes = base64.b64decode(mp3_b64)

            # Forward to RobotControlService.
            # RobotControlService will call controller.play_audio_track(...)
            # which will eventually use the existing WebRTC connection.
            self.robot_control_service.handle_tts_audio(
                mp3_bytes=mp3_bytes,
            ...
            self.get_logger().info(
                f"[TTS] Received audio for robot {robot_id}, "
                f"duration={duration_sec:.2f}s, size={len(mp3_bytes)} bytes"
            )

        except Exception as e:
            self.get_logger().error(
                f"Error in _on_tts_audio for robot {robot_id}: {e}"
            )
    ...
    async def run_robot_control_loop(self, robot_id: str) -> None:
        while True:
            try:
                ...
                self.webrtc_adapter.tick_audio(robot_id)

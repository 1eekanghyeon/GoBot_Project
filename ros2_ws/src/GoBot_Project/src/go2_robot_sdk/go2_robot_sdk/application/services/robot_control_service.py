...
class RobotControlService:
    ...
    def handle_tts_audio(self, mp3_bytes: bytes, duration_sec: float, robot_id: str) -> None:
        """
        Called when TTS audio (already synthesized MP3) should be played on the robot.

        mp3_bytes: raw MP3 data from TTS (not base64, 실제 바이너리)
        duration_sec: estimated length in seconds
        robot_id: which robot to play on ("0" in single-robot mode)
        """
        try:
            # Delegate to the actual controller (WebRTCAdapter)
            self.controller.play_audio_track(robot_id, mp3_bytes, duration_sec)
            logger.info(
                f"TTS audio dispatched to robot {robot_id} "
                f"({duration_sec:.2f}s)"
            )
        except Exception as e:
            logger.error(f"Error handling TTS audio for robot {robot_id}: {e}")